...
@st.cache_data(ttl=60 * 60 * 6)  # 6 saatte bir tazelenir
def evds_risksiz_faiz_getir(api_key):
    """TCMB EVDS'ten Ağırlıklı Ortalama Fonlama Maliyeti (TP.APIFON4) serisinin en güncel
    değerini çekip ondalık oran (örn. 0.40) olarak döner. Başarısızsa None döner."""
    if not api_key:
        return None
    try:
        import requests
        bugun = pd.Timestamp.today()
        baslangic = (bugun - pd.Timedelta(days=45)).strftime("%d-%m-%Y")
        bitis = bugun.strftime("%d-%m-%Y")
        yanit = requests.get(
            "https://evds2.tcmb.gov.tr/service/evds",
            params={"series": "TP.APIFON4", "startDate": baslangic, "endDate": bitis, "type": "json"},
            headers={"key": api_key},
            timeout=10,
        )
        yanit.raise_for_status()
        veri = yanit.json().get("items", [])
        if not veri:
            return None
        df_evds = pd.DataFrame(veri)
        deger_kolonlari = [c for c in df_evds.columns if c not in ("Tarih", "UNIXTIME")]
        if not deger_kolonlari:
            return None
        son_gecerli = pd.to_numeric(df_evds[deger_kolonlari[0]], errors='coerce').dropna()
        if son_gecerli.empty:
            return None
        return float(son_gecerli.iloc[-1]) / 100.0
    except Exception:
        return None
...
if ham_df.empty:
    ...
    try:
        evds_anahtar = st.secrets.get("EVDS_API_KEY", None)
    except Exception:
        evds_anahtar = None
    if not evds_anahtar:
        evds_anahtar = st.sidebar.text_input(
            "TCMB EVDS API Anahtarı (opsiyonel)", type="password",
            help="evds2.tcmb.gov.tr üzerinden ücretsiz alınabilir. Boş bırakılırsa risksiz faiz elle girilir."
        )

    otomatik_faiz = evds_risksiz_faiz_getir(evds_anahtar) if evds_anahtar else None

    if otomatik_faiz is not None:
        st.sidebar.caption(f"TCMB EVDS güncel fonlama maliyeti: %{otomatik_faiz * 100:.2f}")
        faiz_kaynagi = st.sidebar.radio(
            "Risksiz faiz kaynağı", ["TCMB EVDS (otomatik)", "Elle gir"], index=0, key="faiz_kaynak"
        )
        if faiz_kaynagi == "TCMB EVDS (otomatik)":
            risksiz_faiz = otomatik_faiz
        else:
            risksiz_faiz = st.sidebar.number_input(
                "Yıllık risksiz faiz (%)", value=round(otomatik_faiz * 100, 2), step=1.0
            ) / 100.0
    else:
        if evds_anahtar:
            st.sidebar.caption("⚠️ EVDS'ten veri çekilemedi, elle giriş kullanılıyor.")
